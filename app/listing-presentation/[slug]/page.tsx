...
import { buildScenarios, wealthSavings, totalInterestAndMI, fmtDollars, fmtRate, TCAInputs, LoanScenario } from '@/lib/openHouseMath'
...
function PDFViewer({ url }: { url: string }) {
  return (
    <div style={{ width: '100%', borderRadius: 14, overflow: 'hidden', border: `1px solid ${C.border}`, background: C.white }}>
...
// ─── TCA Charts ───────────────────────────────────────────────────────────────
function MonthlySavingsChart({ scenarios }: { scenarios: LoanScenario[] }) {
  const market = scenarios[0]
  const allScenarios = scenarios
  const maxSavings = Math.max(...allScenarios.slice(1).map(s => market.monthlyTotal - s.monthlyTotal), 1)
    ...
    <div style={{ background: C.white, borderRadius: 14, border: `1px solid ${C.border}`, padding: 24 }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
        ...
        {allScenarios.map((s, i) => {
          ...
          const barH = Math.max(2, (Math.max(savings, 0) / maxSavings) * cH)
          const spacing = cW / allScenarios.length
          const cx = PAD.left + spacing * i + spacing / 2
          const barColor = i === 0 ? '#D1D5DB' : C.accent
          const barX = cx - barW / 2
          const barY = PAD.top + cH - barH
          return (
            <g key={s.label}>
              <rect x={barX} y={barY} width={barW} height={barH} fill={barColor} rx="4" />
              <text x={cx} y={barY - 6} textAnchor="middle" fontSize="11" fontWeight="700" fill={i === 0 ? C.muted : C.navy}>
...
function SavingsOverTimeChart({ scenarios }: { scenarios: LoanScenario[] }) {
  const market = scenarios[0]
  const wealthVals = scenarios.map((s, i) => i === 0 ? 0 : wealthSavings(market, s, 120))
  const maxW = Math.max(...wealthVals, 1)
  const W = 560, H = 200, PAD = { top: 30, right: 20, bottom: 40, left: 80 }
  const cW = W - PAD.left - PAD.right
  const cH = H - PAD.top - PAD.bottom
  const barW = Math.min(80, cW / scenarios.length - 16)
    ...
    <div style={{ background: C.white, borderRadius: 14, border: `1px solid ${C.border}`, padding: 24 }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
        ...
        {scenarios.map((s, i) => {
          ...
          const spacing = cW / scenarios.length
          const cx = PAD.left + spacing * i + spacing / 2
...
function InterestMIChart({ scenarios }: { scenarios: LoanScenario[] }) {
  const totals = scenarios.map(s => totalInterestAndMI(s))
...
interface PageData {
  ...
  ufmip_pct: number | null; annual_mip_pct: number | null
  ...
  function openFlyer() {
    ...
    function summaryRows() {
      ...
      const savingsRow = `<tr style="background:#F0FDF4">
        ...
        ${savingsVals.map(v => `<td style="padding:5px 8px;text-align:right;font-weight:800;color:${v > 0 ? '#16a34a' : '#6B7280'};font-size:10px">${v > 0 ? `+${fmtDollars(v)}/mo` : '$0.00'}</td>`).join('')}
      </tr>`
      const ws120Row = `<tr style="background:#F0FDF4">
        ...
        ${ws120Vals.map(v => `<td style="padding:5px 8px;text-align:right;font-weight:800;color:${v > 0 ? '#16a34a' : '#6B7280'};font-size:10px">${v > 0 ? `+${fmtDollars(v)}` : '$0'}</td>`).join('')}
      </tr>`
...
<head>
<meta charset="utf-8">
<title>Listing Presentation — ${page.address}</title>
<style>
...
.partner-corner {
  flex: 0 0 38%; background: ${NEO_CYAN};
...
.partner-logo-placeholder {
  ...
  text-transform: uppercase; letter-spacing: 0.08em; text-align: center; line-height: 1.2;
...
.price-left { width: 62%; display: flex; align-items: center; gap: 14px; padding: 0 22px; border-right: 1px solid rgba(91,203,245,0.25); }
...
.price-right { width: 38%; display: flex; align-items: center; justify-content: center; gap: 8px; padding: 0 18px; }
.sa-badge { font-size: 11px; font-weight: 700; color: ${NEO_CYAN}; letter-spacing: 0.06em; text-transform: uppercase; }
...
.contact-bar {
  height: 1.25in; background: ${NEO_NAVY};
...
<div class="hero">
  ...
  <div class="partner-corner">
    <div class="partner-logo-wrap">
      ${partnerLogo ? `<img src="${partnerLogo}" alt="${page.partner_name}" />` : `<div class="partner-logo-placeholder">Partner<br>Logo</div>`}
    </div>
...
<div class="main-content">
  <div class="left-col">
    ...
    <div class="table-section">
      <div class="table-title">Seller Advantage — Total Cost Analysis</div>
      <table>
        <thead>
          <tr>
            <th>Summary</th>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 20px 60px' }}>
        ...
        {activeTab === 'tca' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            {hasSA ? (
              <>
                {/* ── PDF Presentation (hard-coded template) ───────────── */}
                {SELLER_ADVANTAGE_PDF_URL && (
                  <PDFViewer url={SELLER_ADVANTAGE_PDF_URL} />
                )}

                {/* ── TCA Section Header ───────────────────────────────── */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  <div style={{ height: 2, flex: 1, background: C.border }} />
                  <div style={{ fontSize: 13, fontWeight: 700, color: C.navy, textTransform: 'uppercase', letterSpacing: '0.1em', whiteSpace: 'nowrap' }}>Total Cost Analysis</div>
                  <div style={{ height: 2, flex: 1, background: C.border }} />
                </div>

                {/* ── External TCA embed (MortgageCoach or similar) ───── */}
                {page.tca_url && (
                  <div style={{ background: C.white, borderRadius: 14, border: `1px solid ${C.border}`, overflow: 'hidden' }}>
                    <div style={{ padding: '14px 20px', background: C.navy, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div style={{ color: '#fff', fontWeight: 700, fontSize: 15 }}>TCA Report</div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 480 }}>
                      <thead>
                        <tr style={{ background: C.navy }}>
                          <th style={{ textAlign: 'left', padding: '12px 20px', color: 'rgba(255,255,255,0.6)', fontWeight: 700 }}> </th>
                          {scenarios.map(s => (
                            <th key={s.label} style={{ textAlign: 'right', padding: '12px 20px', color: s.color, fontWeight: 700, whiteSpace: 'nowrap' }}>{s.label}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {[
                          { label: 'Purchase Price', values: scenarios.map(s => fmtDollars(s.purchasePrice)) },
                          { label: 'Down Payment', values: scenarios.map(s => fmtDollars(s.purchasePrice * inputs.downPct)) },
                          { label: 'Loan Amount', values: scenarios.map(s => fmtDollars(s.loanAmount)) },
                          { label: 'Interest Rate', values: scenarios.map(s => {
                            const rateStr = s.isArm ? `${fmtRate(s.rate)} (${inputs.saArmYears}yr ARM)` : fmtRate(s.rate)
                            return s.apr ? `${rateStr}  (APR ${fmtRate(s.apr)})` : rateStr
                          }) },
                          { label: 'Term', values: scenarios.map(() => '30 years') },
                          { label: 'Monthly Payment', values: scenarios.map(s => fmtDollars(s.monthlyTotal) + '/mo') },
                        ].map((row, i) => (
                ...
                {/* ── Charts ───────────────────────────────────────────── */}
                <MonthlySavingsChart scenarios={scenarios} />
                <SavingsOverTimeChart scenarios={scenarios} />
                <InterestMIChart scenarios={scenarios} />
