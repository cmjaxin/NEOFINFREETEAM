...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!data) {
    return { title: 'Open House | NEO Home Loans' }
  }

  const location = [data.city, data.state].filter(Boolean).join(', ')
  const title = `${data.address}${location ? ', ' + location : ''}, Open House | NEO Home Loans`
  const price = data.list_price > 0
    ? `$${Math.round(data.list_price).toLocaleString()} · `
    : ''
  const description = data.description
    ? data.description.slice(0, 155)
    : `${price}${data.address}${location ? ', ' + location : ''}. View this open house powered by NEO Home Loans.`
