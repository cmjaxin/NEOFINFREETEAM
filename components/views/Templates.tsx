...
export default function Templates() {
  ...
  return (
    <div style={{ maxWidth: 1080, margin: '0 auto', padding: 'clamp(16px, 4vw, 40px)' }}>
      <style>{`
        @media (max-width: 639px) {
          .tpl-grid { grid-template-columns: 1fr !important; }
          .tpl-card-header { flex-direction: column !important; align-items: flex-start !important; gap: 10px !important; }
          .tpl-card-header button { width: 100% !important; }
        }
      `}</style>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontWeight: 800, letterSpacing: '-.02em', fontSize: 'clamp(22px,5vw,32px)', margin: 0, color: '#0A2540' }}>Message Templates</h1>
      ...
      <div className="tpl-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <div style={{ background: '#fff', border: '1px solid #E4E8EC', borderRadius: 16, padding: 22, display: 'flex', flexDirection: 'column' }}>
          <div className="tpl-card-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
            <div>
